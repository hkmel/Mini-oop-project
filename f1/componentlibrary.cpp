...
ComponentLibrary::ComponentLibrary(){
    ...
    libraryData["Digital"]=QStringList()<<"AND Gate"<<"OR Gate"<<"NOT Gate"<<"NAND Gate"<<"XOR Gate"<<"D Flip-Flop";
    libraryData["Sources"]=QStringList()<<"GND"<<"Voltage Source";
}
ComponentLibrary::~ComponentLibrary(){}
...
Component*ComponentLibrary::createComponent(const QString&type,const QString&id,const QPointF&pos){
    ...
    if(type=="Voltage Source")return new VoltageSourceComponent(id,pos,5.0);
    ...
    if(type=="AND Gate")return new AndGateComponent(id,pos);
    if(type=="OR Gate")return new OrGateComponent(id,pos);
    if(type=="NOT Gate")return new NotGateComponent(id,pos);
    if(type=="NAND Gate")return new NandGateComponent(id,pos);
    if(type=="XOR Gate")return new XorGateComponent(id,pos);
